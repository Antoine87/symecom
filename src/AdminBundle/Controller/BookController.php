...
class BookController extends Controller
{
    ...
    {
        $book = new Book();
        $form = $this->createForm('ModelBundle\Form\BookType', $book);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('livre_show', array('id' => $book->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($book);
        $editForm = $this->createForm('ModelBundle\Form\BookType', $book);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('livre_edit', array('id' => $book->getId()));
        }
    ...
    private function createDeleteForm(Book $book)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('livre_delete', array('id' => $book->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
